...
export default function NoticeManager() {
  ...
  const noticeTypeIcon = (type: string | undefined): LucideIcon => {
    switch (type) {
      ...
      case 'intimation': return FileText;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Track and respond to tax notices</p>
        </div>
      ...
      {showUpload && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 text-center">
          ...
          <p className="text-sm text-slate-600 mb-3">Upload a notice document for processing</p>
          <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-slate-500 mt-2">Loading notices...</p>
          </div>
        ) : noticeList.length === 0 ? (
          <div className="p-12 text-center">
            <AlertTriangle className="w-10 h-10 text-slate-300 mx-auto mb-3" />
            <p className="text-slate-600 font-medium">No notices found</p>
            <p className="text-sm text-slate-400 mt-1">Upload a notice to get started</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-3 mt-0.5">
                      {notice.client_name && <span className="text-xs text-slate-400">{notice.client_name}</span>}
                      {notice.section && <span className="text-xs text-slate-400">Section {notice.section}</span>}
                      <span className="text-xs text-slate-400">{formatDate(notice.notice_date || notice.created_at)}</span>
                    </div>
                  </div>
                  {notice.response_due_date && (
                    <div className="hidden sm:block text-right shrink-0">
                      <p className="text-xs text-slate-400">Response Due</p>
                      <p className="text-xs font-medium text-slate-600">{formatDate(notice.response_due_date)}</p>
                    </div>
                  )}
                  <span className={`px-2 py-0.5 text-xs font-medium rounded-full shrink-0 ${statusColor(notice.status || 'pending')}`}>
      ...
      {/* Notice detail modal */}
      {selectedNotice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setSelectedNotice(null)}>
          <div
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              ...
              <button onClick={() => setSelectedNotice(null)} className="p-1 text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-slate-700 font-medium">{selectedNotice.client_name || '--'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Section</p>
                  <p className="text-slate-700 font-medium">{selectedNotice.section || '--'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Assessment Year</p>
                  <p className="text-slate-700 font-medium">{selectedNotice.assessment_year || '--'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Notice Date</p>
                  <p className="text-slate-700">{formatDate(selectedNotice.notice_date || selectedNotice.created_at)}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Response Due</p>
                  <p className="text-slate-700 font-medium">{formatDate(selectedNotice.response_due_date)}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">DIN</p>
                  <p className="text-slate-700 font-mono text-xs">{selectedNotice.din || '--'}</p>
                </div>
              </div>
              ...
              {selectedNotice.draft_response && (
                <div>
                  ...
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">
                    {selectedNotice.draft_response}
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-200">
                {selectedNotice.status === 'pending' && (
                  <button
                    ...
                    className="px-4 py-2 bg-purple-500 hover:bg-purple-600 disabled:bg-purple-300 text-white text-sm font-medium rounded-lg flex items-center gap-2"
                  >
                ...
                <button
                  ...
                  className="px-4 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white text-sm font-medium rounded-lg flex items-center gap-2"
                >
