  ...
  if (isError || !client) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-slate-600 font-medium">Client not found</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold text-slate-800">{client.firm_name || client.name}</h1>
            <div className="flex flex-wrap gap-x-6 gap-y-2 mt-3 text-sm text-slate-600">
              {client.pan && (
                <span className="flex items-center gap-1.5">
                  <CreditCard className="w-4 h-4 text-slate-400" /> PAN: <span className="font-mono font-medium">{client.pan}</span>
                </span>
              )}
              {client.gstin && (
                <span className="flex items-center gap-1.5">
                  <Hash className="w-4 h-4 text-slate-400" /> GSTIN: <span className="font-mono font-medium">{client.gstin}</span>
                </span>
              )}
              {client.entity_type && (
                <span className="flex items-center gap-1.5">
                  <Building2 className="w-4 h-4 text-slate-400" /> {entityLabel(client.entity_type)}
                </span>
              )}
            </div>
            <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2 text-sm text-slate-500">
              {client.contact_person && (
                <span className="flex items-center gap-1.5">
                  <User className="w-4 h-4 text-slate-400" /> {client.contact_person}
                </span>
              )}
              {client.email && (
                <span className="flex items-center gap-1.5">
            ...
            {docList.length === 0 ? (
              ...
              <div className="space-y-2">
                {docList.slice(0, 5).map((doc, i) => (
                  <div key={doc.id || i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50">
                    <FileText className="w-4 h-4 text-slate-400" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-slate-700 truncate">{doc.filename || doc.name}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          {taskList.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-slate-600 font-medium">No compliance tasks</p>
              <p className="text-sm text-slate-400 mt-1">Generate a compliance calendar to get started</p>
            </div>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase hidden sm:table-cell">Due Date</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {taskList.map((task, i) => (
                  <tr key={task.id || i} className="hover:bg-slate-50">
                    <td className="px-5 py-3">
                      <p className="text-sm font-medium text-slate-700">{task.task_name || task.name}</p>
                      <p className="text-xs text-slate-400 sm:hidden">{formatDate(task.due_date)}</p>
                    </td>
                    <td className="px-5 py-3 text-sm text-slate-600 hidden sm:table-cell">{formatDate(task.due_date)}</td>
                    <td className="px-5 py-3 text-sm text-slate-600 hidden md:table-cell">{task.task_type || '--'}</td>
                    <td className="px-5 py-3">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          {docList.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">Upload documents from the Documents page</p>
            </div>
          ) : (
            <table className="w-full">
              <thead>
              ...
              <tbody className="divide-y divide-slate-100">
                {docList.map((doc, i) => (
                  <tr key={doc.id || i} className="hover:bg-slate-50">
                    <td className="px-5 py-3">
                      <p className="text-sm font-medium text-slate-700 truncate max-w-xs">{doc.filename || doc.name}</p>
                    </td>
                    <td className="px-5 py-3 text-sm text-slate-600 hidden sm:table-cell">{doc.document_type || doc.type || '--'}</td>
                    <td className="px-5 py-3 text-sm text-slate-600 hidden md:table-cell">{formatDate(doc.uploaded_at || doc.created_at)}</td>
                    <td className="px-5 py-3">
